...
- `config/hypr/hyprlock.conf` and `config/hypr/hypridle.conf` are symlinked as **files** into `~/.config/hypr/` — the dir is managed by the opendwm Wayland installer and also holds `hyprland.lua`. `hypridle` locks after 5 idle minutes and suspends after 15 via `bin/lock-screen`; video playback's idle-inhibit lock pauses both.
